...
const StatsCard: React.FC<StatsCardProps> = ({ title, value, trend, trendColor, icon: Icon, iconColor, periodInfo }) => {    
    return (
        <div className="border border-border bg-card p-6 rounded-2xl shadow-md transition-all duration-300 hover:shadow-glow cursor-pointer">
            ...
            <h3 className={`mt-2 text-3xl font-bold ${title.toLowerCase().includes('revenue') ? 'text-primary' : title.toLowerCase().includes('expense') ? 'text-red-500' : ''}`}>{value}</h3>    
            {trend && (
                <p className={`mt-1 text-sm flex items-center gap-1 ${trendColor === "green" ? "text-primary" : trendColor === "red" ? "text-red-500" : "text-gray-500"}`}>
